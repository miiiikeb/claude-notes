...
(function () {
  ...
  function renderNotes(notes) {
    ...
    list.querySelectorAll('.note-row').forEach(row => {
      row.addEventListener('click', () => navigate('noteDetail', { id: row.dataset.id }));
      row.addEventListener('keydown', e => { if (e.key === 'Enter') navigate('noteDetail', { id: row.dataset.id }); });
    });
  }
  ...
  async function fetchAndRender() {
    ...
    const notes = await api(`/api/notes${qs}`);
    renderNotes(notes);
  }
